...
func TestLoad_Defaults(t *testing.T) {
	...
	require.Equal(t, "http://localhost:3001", cfg.FrontendURL)
...
func TestLoadRequiresAuthPepperInProduction(t *testing.T) {
	t.Setenv("ECHOSTATE_ENV", "production")
	t.Setenv("DATABASE_URL", "postgres://user:pass@localhost:5432/echostate?sslmode=disable")
	t.Setenv("ECHOSTATE_AUTH_PEPPER", "")

	_, err := Load()
	require.Error(t, err)
}

func TestLoadRejectsDefaultAuthPepperInProduction(t *testing.T) {
	t.Setenv("ECHOSTATE_ENV", "production")
	t.Setenv("DATABASE_URL", "postgres://user:pass@localhost:5432/echostate?sslmode=disable")
	t.Setenv("ECHOSTATE_AUTH_PEPPER", "echostate-dev-pepper")

	_, err := Load()
	require.Error(t, err)
}

func TestLoadAllowsStrongAuthPepperInProduction(t *testing.T) {
	t.Setenv("ECHOSTATE_ENV", "production")
	t.Setenv("DATABASE_URL", "postgres://user:pass@localhost:5432/echostate?sslmode=disable")
...
func TestLoadAllowsMissingAuthPepperInDevelopment(t *testing.T) {
